    ...
    for n_walk in range(num_walks):
        ...
        for source in shuffled_nodes:
            ...
            while len(walk) < walk_length:
                ...
                all_possible_groups = [
                    group
                    for group in group2neighbors
                    if len(group2neighbors[group]) > 0
                ]
                ...
                if len(walk) == 1:  # For the first step
                    ...
                    probabilities = d_graph[walk[-1]][probabilities_key][random_group][
                        walk[-2]
                    ]
                    walk_to = np.random.choice(walk_options, size=1, p=probabilities)[0]
...
class FairWalk:
    ...
    def fit(
        ...
        self.lgreg_label = LogisticRegression(
        ...
        ).fit(self.embs[idx_train], sens.squeeze()[idx_train])
    ...
    def _precompute_probabilities(self):
        ...
        for source in nodes_generator:
            for current_node in self.graph.neighbors(source):
                ...
                for destination in self.graph.neighbors(current_node):
                    ...
                    if destination == source:  # Backwards probability
                        ...
                        destination in self.graph[source]
                    ...
                    neighbor_groups.append(
                        self.graph.nodes[destination][self.GROUP_KEY]
                    )
                ...
                for group in groups:
                    cur_unnormalized_weights = unnormalized_weights[
                        neighbor_groups == group
                    ]
                    cur_d_neighbors = d_neighbors[neighbor_groups == group]

                    # Normalize
                    d_graph[current_node][self.PROBABILITIES_KEY][group][source] = (
                        cur_unnormalized_weights / cur_unnormalized_weights.sum()
                    )
            ...
            for destination in self.graph.neighbors(source):
                ...
                first_travel_neighbor_groups.append(
                    self.graph.nodes[destination][self.GROUP_KEY]
                )
            ...
            for group in groups:
                cur_first_travel_weights = first_travel_weights[
                    first_travel_neighbor_groups == group
                ]
                d_graph[source][self.FIRST_TRAVEL_KEY][group] = (
                    cur_first_travel_weights / cur_first_travel_weights.sum()
                )
    ...
    def _generate_walks(self) -> list:
        ...
        walk_results = Parallel(
            n_jobs=self.workers, temp_folder=self.temp_folder, require=self.require
        )(
