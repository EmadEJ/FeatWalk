...
# Note: The FairWalk fit method does not use the `feats` parameter.
# The number of epochs is hardcoded to 50 in your FairWalk.py file.
model.fit(adj=adj,
          labels=labels,
          idx_train=idx_train,
          sens=sens,
...
# Calculate the Individual Fairness metric on the embeddings from FairWalk
if_score = calculate_individual_fairness(model.embs, feats_np)

print("\n--- BASELINE Evaluation Results (FairWalk) ---")
